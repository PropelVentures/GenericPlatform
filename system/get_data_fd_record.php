...
function Get_Data_FieldDictionary_Record($dd_position, $table_alias, $display_page, $tab_status = 'false', $tab_num = 'false', $editable = 'true') {
    ...
    if ($tab_status == 'true') {
		if($top_dd){ // top
			$rs = $con->query("SELECT * FROM data_dictionary WHERE display_page = '$display_page' AND (dd_component_location = 'top' OR dd_component_location = '' OR dd_component_location IS NULL) AND table_type IN(".DD_DEFAULT_TOP.") ORDER BY tab_num");
		} else if($body_dd){ // body
			if(defined('DD_DEFAULT_BOTTOM') && DD_DEFAULT_BOTTOM != "") {
				$rs = $con->query("SELECT * FROM data_dictionary WHERE display_page = '$display_page' AND (dd_component_location = 'body' OR dd_component_location = '' OR dd_component_location IS NULL OR dd_component_location != 'bottom') AND table_type NOT IN(".DD_DEFAULT_TOP.") AND table_type NOT IN(".DD_DEFAULT_BOTTOM.") ORDER BY tab_num");
			} else {
				$rs = $con->query("SELECT * FROM data_dictionary WHERE display_page = '$display_page' AND (dd_component_location = 'body' OR dd_component_location = '' OR dd_component_location IS NULL OR dd_component_location != 'bottom') AND table_type NOT IN(".DD_DEFAULT_TOP.") ORDER BY tab_num");
			}
		} else if($bottom_dd){ // bottom
			if(defined('DD_DEFAULT_BOTTOM') && DD_DEFAULT_BOTTOM != "") {
				$rs = $con->query("SELECT * FROM data_dictionary WHERE display_page = '$display_page' AND (dd_component_location = 'bottom' OR table_type IN(".DD_DEFAULT_BOTTOM.")) AND table_type NOT IN(".DD_DEFAULT_TOP.") ORDER BY tab_num");
			} else {
				$rs = $con->query("SELECT * FROM data_dictionary WHERE display_page = '$display_page' AND dd_component_location = 'bottom' AND table_type NOT IN(".DD_DEFAULT_TOP.") ORDER BY tab_num");
			}
		}
