...
function formating_Update($row, $method, $urow, $image_display = 'false', $page_editable = 'false') {
    ...
    if ($method != 'transaction') {
		...
		$component_type = trim(strtoupper($row['component_type']));
        ...
        if (trim($row['table_type']) != 'transaction') {
            ...
            if ($DD_EDITABLE_TEMP != '11' || $row['editable'] == 'false' || $page_editable == false) {
				...
				if(!empty($row['view_operations'])){
					$single_line_alignment = getAlignmentClass($row['view_operations']);
				}
            }else{
				if(!empty($row['edit_operations'])){
					$single_line_alignment = getAlignmentClass($row['edit_operations']);
				}
			}
        }
    ...
    if ($userPrivilege === true) {
        ...
        switch ($formatArray[0]) {
            case "richtext":
                echo "<div class='new_form $single_line_alignment $fd_css_class' style='$fd_css_style'><div><label class='$fd_css_class' style='$fd_css_style'>$row[field_label_name]</label>";
			...
			case "multi_dropdown":
				echo "<div class='new_form $single_line_alignment $fd_css_class' style='$fd_css_style'><div><label>$row[field_label_name]</label>";
				if ($urow != 'false')
            ...
            case "list_fragment":
                echo "<div class='$single_line_alignment $fd_css_class' style='overflow:auto!important;max-height:500px; $fd_css_style'><div><label class='$fd_css_class' style='$fd_css_style'>$row[field_label_name]</label>";
                if ($urow != 'false')
                    list_fragment($row);
                echo "</div></div><br>";
                break;
            ...
            case "checkbox":
                echo "<div class='new_form $single_line_alignment $fd_css_class' style='$fd_css_style'><div><label class='boolen_label' class='$fd_css_class' style='$fd_css_style'>$row[field_label_name]</label>";
                if ($urow != 'false')
            ...
            case "video":
				showVideo($row,$single_line_alignment,$fd_css_class,$fd_css_style,$field,$fieldValue,$readonly,$required,$inputSize);
				break;
			...
			case "ppt":
				showPPT($row,$single_line_alignment,$fd_css_class,$fd_css_style,$field,$fieldValue,$readonly,$required,$inputSize);
				break;
            ...
            default :
				...
				if (isKeyField($row)) {
					...
					echo "<div class='new_form $single_line_alignment $fd_css_class' style='$fd_css_style'><div><label>$row[field_label_name]</span></label>";
...
function parseFieldType($row) {
    ...
    $result = $con->query("describe $row[table_name]");
...
/**
 * Get length based on column definition in database
 * 
...
function tagFnc($row, $urow = 'false', $image_display = 'false', $dimStyle, $dimWidth) {
    ...
    $list = getMulti($row['table_name'], "$row[generic_field_name] != '' and $row[generic_field_name] != 'NULL'", $row[generic_field_name]);
...
function dropdown($row, $urow = 'false', $fieldValue = 'false', $page_editable = 'check') {
    ...
    if (trim($row['component_type']) != 'transaction') {
        if (( $row['dd_editable'] != '11' && $urow != 'false' ) || $page_editable == false || $row['strict_disabled'] == 'disabled')
    ...
    if ($urow == 'list_display') {
        $qry = $con->query("SELECT $list_fields FROM  $dd[table_name] where $key='$fieldValue'");
        ...
        $qry = $con->query("SELECT $list_fields FROM  $dd[table_name] $order");
...
function list_fragment($row2) {
  ...
  $con = connect();
    ...
    $query = get_listFragment_record($dd['table_name'], $dd['keyfield'], $dd['list_filter'], $dd['list_extra_options'], $fields);
