...
if ($nav->num_rows > 0) {
    $navRecord = $nav->fetch_assoc();
    $displayPage = $navRecord['target_display_page'];
    var_dump($displayPage);
}

if (!showLinkedInProfile(LINKEDIN_IMPORT_PROFILE_APP_REDIRECT_URL, $_SESSION['linkedInCSRFImport'])) {
    $csrf = getUniqueRandomToken(24);
    $_SESSION['linkedInCSRFImport'] = $csrf;
    $authUrl = "https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=" . LINKEDIN_APP_ID . "&redirect_uri=" . LINKEDIN_IMPORT_PROFILE_APP_REDIRECT_URL . "&state=" . $csrf . "&scope=" . LINKEDIN_APP_PERMISSIONS;
    ?>
